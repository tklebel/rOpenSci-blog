...
# Step 1: Hacking Away
Back in March 2017, I was starting out as a student in a research project
...
more and more complex or sometimes quite ridicoulus functions like:
...
would take 7.2 seconds for 3000 files. For 200,000 files of sociological 
articles this would amount to `r 7.2*200000/3000/60` hours. And importing and
converting to a list took a lot of time too. Although I didn't thought, I would
need to import the files many times (which I had to do later to update results
...
to carry the original data home). Apperently, this is not as easy as it sounds.
...
of the function, around 30% of the runtime is spent on reading the file, while 
...
The work on re-writing progressed quickly. The first step was to simply extract the
identifier for the journal within the article it was published:
...
I had to benchmark possibilities to figure it out:
...
`fun3` is quite substantial, if you want to do it 500,000 times: 4.5 minutes. 
I'd guess that `fun2` is faster, since it only calls two functions, and not 
...
I dug deeper again, using the package `profvis` to find bottlenecks. Oddly
...
But how did I find the functions responsible for the delay? I used the package
[profvis](https://rstudio.github.io/profvis/index.html)
a lot during this progress. The package measures time spent line by
line, which let's you identify bottlenecks in your code.
...
may lead to varying results. The function is not measured 500 times, the result
...
profvis::profvis({
  ...
  purrr::rerun(500, new_approach())
})
```


By assessing the efficiency of functions repeatedly and optimising several parts,
...
interface of JSTOR/DfR, execution time is slightly under 4 minutes, or 2 minutes
if executed in parallel, at least on my moderately fast MacBook Pro.
...
# Lesson four: what could I have done differently?
